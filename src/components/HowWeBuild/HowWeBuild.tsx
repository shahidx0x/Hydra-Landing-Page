...
import { setp_details } from "../HowWeBuild/Details";
    ...
    <motion.div initial={{ x: "-100vh", opacity: 0 }} ref={prlx.ref}>
      <div className="flex justify-between mt-5">
        <div className="flex flex-col  gap-2 w-1/2 pl-10">
          <h2 className="font-mono text-2xl lg:text-5xl text-white font-thin">
            HOW WE BUILD
          </h2>
          <div className=" flex gap-5 items-center">
            <p className="font-sans lg:text-2xl text-white">WITH HYDRA VR</p>
        ...
        <div className="lg:flex w-1/2 text-gray-300 blur-0 font-medium break-all pr-10 hidden">
          <h2>
      ...
      <div>
        <div className="lg:flex gap-80 justify-center items-center mt-20 hidden">
          {Array.from({ length: 4 }, (_, index) => (
            <Circle key={index} count={index + 1} />
          ))}
        </div>
        <div className="lg:flex gap-80 justify-center items-center mt-10 hidden">
          {setp_details.map((step, index) => (
            <Steps key={index} details={step.details} />
          ))}
        </div>
